...
import { ArrowRight, MessageSquare, BookOpen, Smartphone } from 'lucide-react';


export default function CallToAction() {
  // URL de imagen abstracta para el fondo
  const backgroundImageUrl = 'https://i.imgur.com/kWsmt3n.png';

  return (
    <section className="py-12 sm:py-20 bg-gradient-to-br from-indigo-600 via-blue-600 to-purple-700 relative overflow-hidden">
      ...
      <div className="relative max-w-6xl mx-auto text-center px-4 sm:px-6 lg:px-8 z-10">
        <div className="flex justify-center mb-6 sm:mb-8">
          {/* Icono de mensaje para reflejar el enfoque en la comunicación */}
          <div className="p-3 sm:p-4 bg-white/20 rounded-xl sm:rounded-2xl backdrop-blur-sm transform hover:scale-110 hover:rotate-12 transition-all duration-500">
            <MessageSquare className="w-8 h-8 sm:w-12 sm:h-12 text-white animate-pulse" />
          </div>
        </div>
        ...
        <h2 className="text-2xl sm:text-3xl md:text-4xl lg:text-6xl font-bold text-white mb-4 sm:mb-6 leading-tight animate-fade-in">
          ...
          <br />
          <span className="text-yellow-300 animate-color-shift">y comenzar a comunicarte?</span>
        </h2>
        ...
        <p className="text-base sm:text-lg md:text-xl lg:text-2xl text-white/90 max-w-3xl mx-auto mb-6 sm:mb-10 leading-relaxed animate-fade-in" style={{ animationDelay: '0.3s' }}>
          Únete a la plataforma que fomenta la autonomía y el aprendizaje inclusivo. Tu camino hacia una **comunicación sin barreras** comienza aquí.
        </p>

        {/* Features highlight */}
        <div className="flex flex-wrap justify-center gap-3 sm:gap-6 mb-6 sm:mb-10 animate-fade-in" style={{ animationDelay: '0.6s' }}>
          {[
            { icon: MessageSquare, text: 'Comunicación Visual' },
            { icon: BookOpen, text: 'Lecciones Diarias' },
            { icon: Smartphone, text: 'Multilenguaje y Accesible' }
          ].map((feature, index) => (
            <div 
              key={index} 
        ...
        <div className="flex flex-col sm:flex-row gap-4 sm:gap-6 justify-center mb-6 sm:mb-10 animate-fade-in" style={{ animationDelay: '1s' }}>
          <Link
            to="/register"
            className="group inline-flex items-center justify-center px-6 sm:px-10 py-3 sm:py-4 bg-white text-indigo-600 text-sm sm:text-lg font-bold rounded-xl sm:rounded-2xl hover:bg-gray-100 transition-all duration-500 transform hover:scale-110 hover:-translate-y-2 shadow-2xl hover:shadow-3xl relative overflow-hidden"
          >
            {/* Shimmer effect */}
            <div className="absolute inset-0 bg-gradient-to-r from-transparent via-indigo-100 to-transparent translate-x-[-100%] group-hover:translate-x-[100%] transition-all duration-1000"></div>
            
            <MessageSquare className="mr-2 sm:mr-3 h-4 w-4 sm:h-6 sm:w-6 group-hover:animate-bounce" />
            <span className="relative z-10 text-xs sm:text-base">Comenzar ahora</span>
        ...
        <div className="mt-8 sm:mt-12 p-4 sm:p-6 bg-white/10 backdrop-blur-sm rounded-xl sm:rounded-2xl transform hover:scale-105 transition-all duration-500 animate-fade-in" style={{ animationDelay: '1.4s' }}>
          <p className="text-white/90 mb-3 sm:mb-4 font-medium text-sm">Disponible en:</p>
          <div className="flex justify-center space-x-4 sm:space-x-6">
            <div className="flex items-center bg-white/20 rounded-lg sm:rounded-xl px-3 sm:px-4 py-1.5 sm:py-2 transform hover:scale-110 hover:bg-white/30 transition-all duration-300">
              ...
              <span className="text-white font-medium text-xs sm:text-sm">Web App (React)</span>
            </div>
            <div className="flex items-center bg-white/20 rounded-lg sm:rounded-xl px-3 sm:px-4 py-1.5 sm:py-2 transform hover:scale-110 hover:bg-white/30 transition-all duration-300">
