...
export default function Register() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (password !== confirmPassword) {
      setError(currentLanguage === 'es' ? 'Las contraseñas no coinciden' : 'Passwords do not match');
      setIsLoading(false);
      return;
    }

    if (password.length < 8) {
      setError(currentLanguage === 'es' ? 'La contraseña debe tener al menos 8 caracteres' : 'Password must be at least 8 characters');
      setIsLoading(false);
      return;
    }
    ...
    try {
      ...
      setError(currentLanguage === 'es' ? 'Error al crear la cuenta. Por favor intenta de nuevo.' : 'Error creating account. Please try again.');
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleGoogleLoginSuccess = async (credentialResponse: any) => {
    ...
    try {
      ...
      setError(err instanceof Error ? err.message : (currentLanguage === 'es' ? 'Error al iniciar sesión con Google' : 'Error signing in with Google'));
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleGoogleLoginError = () => {
    setError(currentLanguage === 'es' ? 'Error al iniciar sesión con Google. Por favor intenta de nuevo.' : 'Error signing in with Google. Please try again.');
  };
  ...
  const translations = {
    es: {
      ...
      joinThousands: 'Únete a miles construyendo hábitos emocionales más saludables',
      name: 'Nombre',
      lastname: 'Apellido',
      ...
      registerWithGoogle: 'O regístrate con email',
      alreadyHaveAccount: '¿Ya tienes una cuenta?',
      login: 'Iniciar sesión',
      loading: 'Cargando...',
      registerWithGoogleBtn: 'Registrarse con Google'
    },
    en: {
      startJourney: 'Start your wellness journey',
      joinThousands: 'Join thousands building healthier emotional habits',
      name: 'First Name',
      lastname: 'Last Name',
      email: 'Email',
      password: 'Password',
      ...
      registerWithGoogle: 'Or register with email',
      alreadyHaveAccount: 'Already have an account?',
      login: 'Login',
      loading: 'Loading...',
      registerWithGoogleBtn: 'Sign up with Google'
    },
    fr: {
      startJourney: 'Commencez votre voyage de bien-être',
      joinThousands: 'Rejoignez des milliers de personnes qui développent des habitudes émotionnelles plus saines',
      name: 'Prénom',
      lastname: 'Nom',
      email: 'Email',
      ...
      registerWithGoogle: 'Ou inscrivez-vous avec email',
      alreadyHaveAccount: 'Vous avez déjà un compte?',
      login: 'Se connecter',
      loading: 'Chargement...',
      registerWithGoogleBtn: 'S\'inscrire avec Google'
    },
    pt: {
      startJourney: 'Comece sua jornada de bem-estar',
      joinThousands: 'Junte-se a milhares construindo hábitos emocionais mais saudáveis',
      name: 'Nome',
      lastname: 'Sobrenome',
      email: 'Email',
      password: 'Senha',
      ...
      registerWithGoogle: 'Ou registre-se com email',
      alreadyHaveAccount: 'Já tem uma conta?',
      login: 'Entrar',
      loading: 'Carregando...',
      registerWithGoogleBtn: 'Cadastrar-se com Google'
    }
  };
    ...
    <div className={`min-h-screen flex flex-col justify-center py-12 sm:px-6 lg:px-8 transition-all duration-500 relative overflow-hidden`}>
      ...
      <div className="absolute inset-0 z-0">
        <motion.div 
          ...
          style={{
            backgroundImage: `url('https://images.unsplash.com/photo-1544367567-0f2fcb009e0b?w=1920&h=1080&fit=crop&crop=center')`
          }}
          animate={{ 
            scale: [1, 1.05, 1],
        ...
        <motion.div 
          ...
          style={{
            backgroundImage: `url('https://images.unsplash.com/photo-1518837695005-2083093ee35b?w=800&h=1200&fit=crop&crop=center')`
          }}
          animate={{ 
        ...
        <div className={`py-8 px-4 shadow-xl sm:rounded-lg sm:px-10 transition-all duration-300 ${
          ...
          >
            ...
            >
              ...
              <span>{t.registerWithGoogleBtn}</span>
            </motion.button>
          ...
          <form className="space-y-6" onSubmit={handleSubmit}>
            {error && (
              <div className={`border px-4 py-3 rounded ${
                theme === 'light'
                  ? 'bg-red-50/90 border-red-200 text-red-700 backdrop-blur-sm'
              ...
              <div className="mt-1">
                <input
                  ...
                  className={`appearance-none block w-full px-3 py-2 border rounded-md shadow-sm focus:outline-none focus:ring-emerald-500 focus:border-emerald-500 sm:text-sm ${
                    theme === 'light'
                      ? 'border-gray-300 placeholder-gray-400 bg-white/90 text-slate-800'
            ...
            <div className="mt-6">
              <Link
                ...
                className={`w-full flex justify-center py-2 px-4 border rounded-md shadow-sm text-sm font-medium transition-colors ${
                  theme === 'light'
                    ? 'border-gray-300 text-slate-700 bg-white/90 hover:bg-white/95'
